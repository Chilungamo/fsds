...
CONCEPT_TAG = "AccountsReceivableNetCurrent"
...
if results:
    ...
    try:
        ...
        df.to_sql("receivables", engine, if_exists="replace", index=False)
        logging.info("✅ Data saved to PostgreSQL table `receivables`")
        
        # Optional: validate
        row_count = pd.read_sql("SELECT COUNT(*) FROM receivables", engine).iloc[0, 0]
