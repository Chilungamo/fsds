...
try:
    with engine.connect() as conn:
        result = conn.execute(text("SELECT cik, accession, ddate FROM assets2"))
...
if results:
    ...
    try:
        df.to_sql("assets2", engine, if_exists="append", index=False)
        logging.info("✅ Data appended to PostgreSQL table `assets2`")
    except Exception as e:
