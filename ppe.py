...
if results:
    ...
    try:
        ...
        df.to_sql("ppe", engine, if_exists="replace", index=False)
        logging.info("✅ Data saved to PostgreSQL table `ppe`")
        
        # Optional: validate
        row_count = pd.read_sql("SELECT COUNT(*) FROM ppe", engine).iloc[0, 0]
